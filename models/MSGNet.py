...
def wavelet_transform(x, level=1):
    B, T, C = x.size()
    coeffs = []
    ...
    for b in range(B):
        for c in range(C):
            ...
            coeff = pywt.wavedec(x[b, :, c].detach().cpu().numpy(), wavelet= 'db6',level=None)
...
class ScaleGraphBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if use_wavelet:
            wavelet_coeffs = wavelet_transform(x)  # Simplified wavelet transform
